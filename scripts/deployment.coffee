...
Helpers                = require "../node_modules/hubot-deploy-heroku/src/helpers"
Octonode               = require "octonode"
Deployment             = Helpers.Deployment
GitHubDeploymentStatus = require("../node_modules/hubot-deploy-heroku/node_modules/hubot-deploy/src/models/github_requests").GitHubDeploymentStatus

class Deployment
  constructor: (@deployment, @herokuToken, @githubToken, @logger) ->
    @description = @deployment.description.match(/from hubot-deploy/)
    @number      = @deployment.id
    @yubikey     = @deployment.yubikey
    @repoName    = @deployment.payload.config.repository
    @appName     = @appName()
    @version     = @deployment.sha[0..8]
